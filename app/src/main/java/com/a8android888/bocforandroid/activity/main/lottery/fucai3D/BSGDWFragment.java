...
/**
 * Created by Administrator on 2017/4/24.//百十个定位
 */
public class BSGDWFragment extends BaseFragment implements View.OnClickListener {

    public SparseArray<ArrayList<LotterycomitorderBean>> sparseArray ;
    private Lottery_bsdw_6mfragment_Adapter twoListAdapter;
    ArrayList<LotterycomitorderBean> jsonstring ;
    ListView lsitview2;
    String[] titles;
...
//
    ...
    private void GetData() {
        ...
        Httputils.PostWithBaseUrl(Httputils.refreshRate, params, new MyJsonHttpResponseHandler(getActivity(), Lottery_MainActivity.needdialog()) {
            ...
            public void onSuccessOfMe(JSONObject jsonObject) {
                ...
                LogTools.ee("jsonObject", jsonObject.toString());
                ...
                if (LotteryFragment1.getGanmeitmexzlxcode().equalsIgnoreCase("BSDW") || LotteryFragment1.getGanmeitmexzlxcode().equalsIgnoreCase("BGDW")
                    ...
                    if (LotteryFragment1.getGanmeitmexzlxcode().equalsIgnoreCase("BSGDW")) {

                        titles = new String[]{LotteryFragment1.getGanmeitmexzlxname().substring(0, 1),
                                LotteryFragment1.getGanmeitmexzlxname().substring(1, 2), LotteryFragment1.getGanmeitmexzlxname().substring(2, 3)};

                    } else {

                        titles = new String[]{LotteryFragment1.getGanmeitmexzlxname().substring(0, 1),
                                LotteryFragment1.getGanmeitmexzlxname().substring(1, 2)};
                    }

                    String pl = "";
                    jsonstring = new ArrayList<LotterycomitorderBean>();
                    for (int i = 0; i < jsonArray.length(); i++) {
                        ...
                        if (jsobject.optString("id").indexOf("-"+LotteryFragment1.getGanmeitmexzlxcode()) != -1) {
                            LogTools.e("点击的是"+jsobject.optString("id", ""),LotteryFragment1.getGanmeitmexzlxcode());
    ...
    public void SetAdapter() {
        if (twoListAdapter == null) {
            twoListAdapter = new Lottery_bsdw_6mfragment_Adapter(getActivity(), sparseArray, titles);
            lsitview2.setAdapter(twoListAdapter);

        } else
            twoListAdapter.NotifyAdapter(sparseArray,titles);
    ...
    public void StartActvity(ArrayList<LotterycomitorderBean> beans) {
        if (beans.size() > 0) {
                ...
                for (int i = 0; i < beans.size(); i++) {
                    if (beans.get(i).getUid().equalsIgnoreCase(titles[i1])) {
                        ...
                        if (sdbNumber.length() > 0) {
                            sdbNumber.append(",").append(twoListAdapter.GetSelectItem().get(i).getNumber());
                        } else {
                            ...
                            sdbNumber.append(twoListAdapter.GetSelectItem().get(i).getUid() + ":");
                            sdbNumber.append(twoListAdapter.GetSelectItem().get(i).getNumber());
                        }
                    }
            ...
            LogTools.e("this.getActivity()", sdbNumber3.toString());
            boolean state = false;
            for (int i2 = 0; i2 < titles.length; i2++) {
            ...
            if(index==titles.length){
                ...
                LotterycomitorderBean gameBean1 = new LotterycomitorderBean();
                LogTools.e("点击的是"+jsonstring.get(0).getUid(),LotteryFragment1.getGanmeitmexzlxcode());
                ...
                ToastUtil.showMessage(this.getActivity(), "只能选择" + titles.length + "行");
            }
        } else {
            ToastUtil.showMessage(this.getActivity(), "只能选择" + titles.length + "行");
        }
