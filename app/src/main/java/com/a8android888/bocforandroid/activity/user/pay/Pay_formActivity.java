...
public class Pay_formActivity extends BaseActivity implements View.OnClickListener {
    TextView title, bankname, banknumber, bankusername;
    ImageView back;
    Button comit;
    TextView choosedate;
    EditText hkfs;
...
//    1:微信 2:支付宝 3:财付通 4:银行卡 5:网页支付
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toplayout.setBackground(new ColorDrawable(Color.rgb(0, 147, 221)));
...
// 将ClipData内容放到系统剪贴板里。
    ...
    private void initview() //这里面有 快捷支付 第一个项 公司入款全部项 isinc false 是公司
    {
        ...
        ImageLoader imageLoader = ((BaseApplication) getApplication()).getImageLoader();
...
//            arrayList.get(0).setHint("最低金额" + data.optString("minEdu"));
        ...
        if (index.equalsIgnoreCase("4"))
            title.setText("银行卡充值");//titles[Integer.valueOf(index) - 1]
        else
            title.setText(data.optString("bankType", "") + "充值");//titles[Integer.valueOf(index) - 1]
...
//        params.put("id", data.optString("id", ""));
    ...
    private void SaveFast(String hkType, String hkName, final boolean needfinish, String remark) {
        ...
        if (indexnum > -1 && indexnum < 4) {
            if (!new verifyEditext().Companypay_Type(this, jine.getText().toString().trim(), choosedate.getText().toString().trim(), hkType, hkName)) {
                return;
            }
        } else if (indexnum == 4) {
            //银行
            if (!new verifyEditext().Companypay_Bank(this, jine.getText().toString().trim(), choosedate.getText().toString().trim(), hkType, hkName)) {
                return;
            }
...
//        }
        ...
        RequestParams params = new RequestParams();
        params.put("userName", ((BaseApplication) getApplication()).getBaseapplicationUsername());
        params.put("money", Httputils.Limit2(money * 1.00d));
        params.put("payNo", data.optString("payNo", ""));
        if (choosedate != null)
            params.put("time", choosedate.getText().toString().trim());//yyyy-MM-dd HH:mm:ss
...
//        params.put("payType", data.optString("payType", ""));
        ...
        stringBuilder.append(((BaseApplication) getApplication()).getBaseapplicationUsername());
        stringBuilder.append("|");
        stringBuilder.append(Httputils.Limit2(money * 1.00d));
        stringBuilder.append("|");
//        if(choosedate!=null) {
