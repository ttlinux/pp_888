...
import com.a8android888.bocforandroid.Adapter.TestAdapter;
...
import com.a8android888.bocforandroid.thirdparty.advrecyclerview.expandable.RecyclerViewExpandableItemManager;
...
//        recycler_view.setAdapter(expMgr.createWrappedAdapter(new TestAdapter()));
...
//        });
    ...
    public void GetData()
    {
        ...
        params.put("timeType","tom");//tom today
        ...
        Httputils.PostWithBaseUrl(Httputils.SportsOdds, params, new MyJsonHttpResponseHandler(getActivity(), true) {
            ...
            public void onSuccessOfMe(JSONObject jsonObject) {
                ...
                LogTools.ee("jsonObject", "testfragment" + jsonObject);
                isRuning = false;
                if (publicDialog != null && getActivity() != null) {
                    publicDialog.dismiss();
    ...
    private void  SetAdapter(int type)
    {
        ...
        if(testAdapter==null)
        {
            if(type==BALL_TYPE.FootBall_Normal.value())
                testAdapter=new TestAdapter(getActivity(),sports_Normal,type);

            else if(type==BALL_TYPE.FootBall_GuoGuan.value())
                testAdapter=new TestAdapter(getActivity(),sports_guoguan,type);

            else if(type==BALL_TYPE.BasketBall_GuoGuan.value())
                testAdapter=new TestAdapter(getActivity(),bkBeans_guoguan,type);

            else if(type==BALL_TYPE.BasketBall_Normal.value())
                testAdapter=new TestAdapter(getActivity(),bkBeans_normal,type);

            if(testAdapter!=null) {
                recycler_view.setAdapter(expMgr.createWrappedAdapter(testAdapter));
            }
        }
        else
        {
            if(type==BALL_TYPE.FootBall_Normal.value())
                testAdapter.NotifyAdapter(sports_Normal,type);

            else if(type==BALL_TYPE.FootBall_GuoGuan.value())
                testAdapter.NotifyAdapter(sports_guoguan,type);

            else if(type==BALL_TYPE.BasketBall_GuoGuan.value())
                testAdapter.NotifyAdapter(bkBeans_guoguan,type);

            else if(type==BALL_TYPE.BasketBall_Normal.value())
                testAdapter.NotifyAdapter(bkBeans_normal,type);
//////////////////////////////////////////////////////////////////////////
            else if(type==BALL_TYPE.FootBall_BanQuan.value())
                testAdapter.NotifyAdapter(footBallHqs,type);

            else if(type==BALL_TYPE.FootBall_ZongRuQiu.value())
                testAdapter.NotifyAdapter(footBallRuQius,type);

            else if(type==BALL_TYPE.FootBall_BoDan.value())
                testAdapter.NotifyAdapter(footBallbodans,type);
