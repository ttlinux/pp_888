 ...
 */
public class CCSSC_zxListAdapter extends BaseAdapter{

    Activity activity;
    SparseArray<?> sparseArray;
    String[] titles;
    SparseArray<LotterycomitorderBean> recordlist=new SparseArray<LotterycomitorderBean>();
    SparseArray<View> linearrs=new SparseArray<View>();

    public CCSSC_zxListAdapter(Activity activity, SparseArray<?> sparseArray, String[] titles)
    {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (titles.length > 1) {
            holder.zm.setText(titles[position]+"位");
        } else {
...
//        hora.setLayoutParams(paramsa);
        for (int i = 0; i < count; i++) {
            ...
            for (int j = i*2; j <tempcount; j++) {
                ...
                View popupView = View.inflate(activity, R.layout.item_zxtmfragement, null);
                ...
                indexclass.setPosition(pos);
                indexclass.setIndex(j);
                peilv.setTag(indexclass);
                peilv.setChecked(beans.get(j).getSelected());
                if(beans.get(j).getPl().equalsIgnoreCase("0")){
                    peilv.setTextSize(13);
                peilv.setText("请选择");}
                else{
                ...
                peilv.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View buttonView) {
                        ...
                        Indexclass indexclass1=(Indexclass)buttonView.getTag();
                        int index = indexclass1.getIndex();
                        int position = indexclass1.getPosition();
                        int tempindex=(index+1)*(position+1);
                        if (box.isChecked() == true) {
                            beans.get(index).setSelected(true);
                            recordlist.put(tempindex,beans.get(index));
...
//        LogTools.e("GetSelectItem",new Gson().toJson(recordlist));
        ArrayList<LotterycomitorderBean> lotterycomitorderBeans=new ArrayList<>();
        for (int i = 0; i <recordlist.size() ; i++) {
            lotterycomitorderBeans.add(recordlist.valueAt(i));
        }
